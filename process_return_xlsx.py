...
def dict_batches_100(data: dict) -> list:
    ...
    Returns:
        list: list of dicts
    """
    key_batches = [[key for key in data][x : x + 100] for x in range(0, len(data), 100)]
...
def extract_data_ya(wb: openpyxl.Workbook) -> dict:
    ...
    canseled = [x[0] for x in data if x[0] in result and x[1] == "Отменён"]
    for i in canseled:
        result.pop(i)
...
async def find_orders_in_crm(
    ...
    while True:
        ...
        url = "https://crm-api/api/v5/orders?limit=100&page=1"
...
async def order_change_status_crm(
    ...
    while True:
        ...
        headers = {"X-API-KEY": CRM_API_KEY}
        url = f"https://crm-api/api/v5/orders/{order_id}/edit?by=externalId"
        order = {
...
async def process_return_xlsx(wb: openpyxl.Workbook, extractor: Callable):
...
async def main():
    wb_yandex = openpyxl.load_workbook("ya_returns.xlsx")
